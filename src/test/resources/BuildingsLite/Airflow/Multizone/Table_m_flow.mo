...
model Table_m_flow
  ...
  extends BuildingsLite.Airflow.Multizone.BaseClasses.PartialOneWayFlowElement(
    m_flow = BuildingsLite.Utilities.Math.Functions.interpolate(
      u=dp,
      xd=dpMea_nominal,
  ...
  parameter Modelica.Units.SI.MassFlowRate mMea_flow_nominal[:]
    ...
    annotation (Dialog(group="Test data"));
protected
  parameter Real[size(dpMea_nominal, 1)] d = BuildingsLite.Utilities.Math.Functions.splineDerivatives(
    x=dpMea_nominal,
    y=mMea_flow_nominal,
