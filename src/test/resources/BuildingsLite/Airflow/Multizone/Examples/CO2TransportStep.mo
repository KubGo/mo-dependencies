...
model CO2TransportStep "Model with transport of CO2 through buoyancy driven flow"
  extends BuildingsLite.Airflow.Multizone.Validation.ThreeRoomsContam(
    volWes(nPorts=5),
    volTop(nPorts=3),
    volEas(nPorts=6));

  BuildingsLite.Fluid.Sensors.TraceSubstances CO2SenTop(
    redeclare package Medium = Medium,
    warnAboutOnePortConnection = false)
    "CO2 sensor"
    annotation (Placement(transformation(extent={{20,120},{40,140}})));
  BuildingsLite.Fluid.Sensors.TraceSubstances CO2SenWes(
    redeclare package Medium = Medium,
    warnAboutOnePortConnection = false)
    "CO2 sensor"
    annotation (Placement(transformation(extent={{-102,10},{-82,30}})));
  BuildingsLite.Fluid.Sensors.TraceSubstances CO2SenEas(
    redeclare package Medium = Medium,
    warnAboutOnePortConnection = false)
    "CO2 sensor"
  ...
  BuildingsLite.Fluid.Sources.TraceSubstancesFlowSource sou(
    redeclare package Medium = Medium,
    use_m_flow_in=true,
    nPorts=1) "CO2 source"
