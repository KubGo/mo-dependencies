within BuildingsLite.Controls.Predictors.Examples;
model BESTEST "This example applies the load prediction to a BESTEST model"
  extends BuildingsLite.ThermalZones.Detailed.Validation.BESTEST.Cases6xx.Case600;
  ElectricalLoad preReg(predictionModel=BuildingsLite.Controls.Predictors.Types.PredictionModel.WeatherRegression)
    "Load prediction based on regression"
    annotation (Placement(transformation(extent={{160,10},{180,30}})));
  Sources.DayType dayType(days={BuildingsLite.Controls.Types.Day.WorkingDay})
