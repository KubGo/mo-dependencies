within BuildingsLite.Controls.Predictors;
block ElectricalLoad "Block that predicts an electrical load"
  ...
  parameter BuildingsLite.Controls.Predictors.Types.PredictionModel
  ...
  if (predictionModel == BuildingsLite.Controls.Predictors.Types.PredictionModel.WeatherRegression)
  ...
  BuildingsLite.Controls.Interfaces.DayTypeInput typeOfDay[integer((nPre-1)/nSam)+2] "Type of day for the current and the future days for which a prediction is to be made.
...
protected
  ...
  discrete output Modelica.Units.SI.Power P[BuildingsLite.Controls.Types.nDayTypes,
  ...
  discrete output Modelica.Units.SI.Temperature T[if predictionModel == Types.PredictionModel.WeatherRegression
     then BuildingsLite.Controls.Types.nDayTypes else 0,if predictionModel == Types.PredictionModel.WeatherRegression
  ...
  Integer iHis[BuildingsLite.Controls.Types.nDayTypes,nSam]
    "Index for power of the current sampling history, for the currrent time interval";
  Boolean historyComplete[BuildingsLite.Controls.Types.nDayTypes,nSam](each start=false, each fixed=true)
  ...
  discrete Modelica.Units.SI.Power PPreHis[BuildingsLite.Controls.Types.nDayTypes,
    nSam] "Predicted power consumptions for all day off time intervals";
  Boolean PPreHisSet[BuildingsLite.Controls.Types.nDayTypes, nSam](each start=false, each fixed=true)
  ...
  function getTypeOfDays
    input Modelica.Units.SI.Time t "Simulation time";
    input BuildingsLite.Controls.Types.Day[:] typeOfDay
...
initial equation
  ...
  P = zeros(
    BuildingsLite.Controls.Types.nDayTypes,
    nSam,
    nHis);
  PPre = zeros(nPre);
  ...
  EActAve    = 0;
  EHisAve    = 0;
  PPreHis    = zeros(BuildingsLite.Controls.Types.nDayTypes, nSam);

  ELast = 0;
  intTOutLast = 0;
  tLast = time;
  iHis = zeros(BuildingsLite.Controls.Types.nDayTypes, nSam);

   _storeHistory = true;
   samStart = BuildingsLite.Controls.Predictors.BaseClasses.sampleStart(
     t=time,
     samplePeriod=samplePeriod);

   for i in 1:nPre loop
      _typeOfDay[i] =  Integer(BuildingsLite.Controls.Types.Day.WorkingDay);
   end for;
  ...
  when sampleTrigger then
    ...
    if predictionModel == Types.PredictionModel.Average then
      for m in 1:nPre loop
        ...
        // in future versions.
        PPre[m] :=BuildingsLite.Controls.Predictors.BaseClasses.average(
    ...
    elseif predictionModel == Types.PredictionModel.WeatherRegression then
      for m in 1:nPre loop
        PPre[m] :=BuildingsLite.Controls.Predictors.BaseClasses.weatherRegression(
...
Moved to package <code>BuildingsLite.Controls.Predictors</code>.
</li>
<li>
