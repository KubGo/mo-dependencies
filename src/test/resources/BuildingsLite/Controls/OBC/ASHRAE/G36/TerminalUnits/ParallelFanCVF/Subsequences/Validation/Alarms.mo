...
model Alarms "Validation of model that generates alarms"
  ...
  BuildingsLite.Controls.OBC.ASHRAE.G36.TerminalUnits.ParallelFanCVF.Subsequences.Alarms ala(
    final staPreMul=1,
    final hotWatRes=1,
  ...
  BuildingsLite.Controls.OBC.CDL.Logical.Sources.Pulse damSta(
    final width=0.5,
  ...
  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Ramp TDis(
    final duration=3600,
    final offset=273.15 + 20,
    final height=-5) "Discharge air temperature"
    annotation (Placement(transformation(extent={{-100,-130},{-80,-110}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Sources.Pulse terFanComOn(
    final width=0.5,
    final period=7500)
    "Terminal fan command on status"
    annotation (Placement(transformation(extent={{-60,70},{-40,90}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Sources.Pulse terFanOn(
    final width=0.5,
    final period=7500,
    shift=1000) "Terminal fan status"
    annotation (Placement(transformation(extent={{-100,40},{-80,60}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Ramp valPos(
    final duration=2000,
    final height=-0.7,
    final offset=0.7,
    final startTime=3600) "Valve position"
    annotation (Placement(transformation(extent={{-60,-60},{-40,-40}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Constant TSup(
    final k=273.15 + 13)
    "AHU supply air temperature"
    annotation (Placement(transformation(extent={{-100,-80},{-80,-60}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Constant TDisSet(
    final k=273.15 + 30)
...
equation
  ...
  connect(terFanOn.y, ala.u1TerFan) annotation (Line(points={{-78,50},{32,50},{
  ...
  connect(intPul.y, ala.uOpeMod) annotation (Line(points={{-38,30},{36,30},{36,
          51},{78,51}},
                    color={255,127,0}));
annotation (
  experiment(StopTime=7200, Tolerance=1e-6),
  __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/ASHRAE/G36/TerminalUnits/ParallelFanCVF/Subsequences/Validation/Alarms.mos"
        "Simulate and plot"),
    Documentation(info="<html>
<p>
This example validates
<a href=\"modelica://BuildingsLite.Controls.OBC.ASHRAE.G36.TerminalUnits.ParallelFanCVF.Subsequences.Alarms\">
