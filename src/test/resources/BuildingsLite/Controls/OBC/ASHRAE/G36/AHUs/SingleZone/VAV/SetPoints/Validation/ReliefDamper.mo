...
model ReliefDamper "Validation of relief damper control"
  ...
  BuildingsLite.Controls.OBC.ASHRAE.G36.AHUs.SingleZone.VAV.SetPoints.ReliefDamper relDam(final
      relDam_min=0.1, final relDam_max=0.6) "Relief damper controller"
    annotation (Placement(transformation(extent={{60,-10},{80,10}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Ramp outDamMin(
    final height=0.1,
    final offset=0.1,
    final duration=3600)
    "Outdoor damper minimum position"
    annotation (Placement(transformation(extent={{-60,50},{-40,70}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Sources.Pulse fanStatus(final width=0.8,
    final period=3600)
    "Supply fan status"
