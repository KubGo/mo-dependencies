...
block ZoneStates "Select the zone state"
  ...
  BuildingsLite.Controls.OBC.CDL.Interfaces.RealInput uHea "Heating control signal"
    annotation (Placement(transformation(extent={{-180,20},{-140,60}}),
        iconTransformation(extent={{-140,20},{-100,60}})));
  BuildingsLite.Controls.OBC.CDL.Interfaces.RealInput uCoo "Cooling control signal"
    annotation (Placement(transformation(extent={{-180,-60},{-140,-20}}),
        iconTransformation(extent={{-140,-60},{-100,-20}})));
  BuildingsLite.Controls.OBC.CDL.Interfaces.IntegerOutput yZonSta "Zone state"
    annotation (Placement(transformation(extent={{140,-20},{180,20}}),
        iconTransformation(extent={{100,-20},{140,20}})));

protected
  BuildingsLite.Controls.OBC.CDL.Conversions.BooleanToInteger booToIntHea(
    final integerFalse=0,
    final integerTrue=BuildingsLite.Controls.OBC.ASHRAE.G36.Types.ZoneStates.heating)
    "Convert Boolean to Integer number"
    annotation (Placement(transformation(extent={{0,30},{20,50}})));
  BuildingsLite.Controls.OBC.CDL.Conversions.BooleanToInteger booToIntCoo(
    final integerFalse=0,
    final integerTrue=BuildingsLite.Controls.OBC.ASHRAE.G36.Types.ZoneStates.cooling)
    "Convert Boolean to Integer number"
    annotation (Placement(transformation(extent={{28,-30},{48,-10}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Nor isDea "In deadband state"
    annotation (Placement(transformation(extent={{30,-80},{50,-60}})));
  BuildingsLite.Controls.OBC.CDL.Conversions.BooleanToInteger booToIntDea(
    final integerFalse=0,
    final integerTrue=BuildingsLite.Controls.OBC.ASHRAE.G36.Types.ZoneStates.deadband)
    "Convert Boolean to Integer number"
    annotation (Placement(transformation(extent={{70,-80},{90,-60}})));
  BuildingsLite.Controls.OBC.CDL.Logical.And isHea "In heating state if both conditions are true"
    annotation (Placement(transformation(extent={{-72,30},{-52,50}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Hysteresis hysUHea(
    final uLow=uLow,
    final uHigh=uHigh)
    "Check if it is in heating state"
    annotation (Placement(transformation(extent={{-120,60},{-100,80}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Hysteresis hysUCoo(
    final uLow=uLow,
    final uHigh=uHigh)
    "Check if it is in cooling state"
    annotation (Placement(transformation(extent={{-100,-50},{-80,-30}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Subtract uHeaMinUCoo
    "Difference between uHea and uCoo"
    annotation (Placement(transformation(extent={{-130,0},{-110,20}})));
  BuildingsLite.Controls.OBC.CDL.Logical.And isCoo "In cooling state if both inputs are true"
    annotation (Placement(transformation(extent={{-10,-30},{10,-10}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Hysteresis hysU(
    final uLow=-uLow,
    final uHigh=uLow)
    "Check if heating control signal is bigger than cooling control signal"
    annotation (Placement(transformation(extent={{-100,0},{-80,20}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Not notHea "Not in heating state"
    annotation (Placement(transformation(extent={{-44,0},{-24,20}})));
  BuildingsLite.Controls.OBC.CDL.Integers.Add addInt "Add two integer inputs"
    annotation (Placement(transformation(extent={{70,20},{90,40}})));
  BuildingsLite.Controls.OBC.CDL.Integers.Add addInt1 "Add two integer inputs"
