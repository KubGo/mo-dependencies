...
model ReturnFanAirflowTracking
  ...
  BuildingsLite.Controls.OBC.ASHRAE.G36.AHUs.MultiZone.VAV.SetPoints.ReturnFanAirflowTracking
  ...
  BuildingsLite.Controls.OBC.CDL.Logical.Sources.Pulse yFan(width=0.75,
    final period=4000) "Supply fan status"
    annotation (Placement(transformation(extent={{-80,60},{-60,80}})));

  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Ramp supFlo(
    final height=1.0,
    final offset=0.2,
    final duration=1800) "Supply air flow rate"
    annotation (Placement(transformation(extent={{-80,10},{-60,30}})));

  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Ramp retFlo(
    final height=0.4,
    final offset=0.1,
