...
block ControlLoops "Heating and cooling control loops"
  ...
  BuildingsLite.Controls.OBC.CDL.Interfaces.RealOutput yCoo(
    final min=0,
    final max=1,
    final unit="1") "Cooling control signal"
    annotation (Placement(transformation(extent={{160,50},{200,90}}),
      iconTransformation(extent={{100,40},{140,80}})));
  BuildingsLite.Controls.OBC.CDL.Interfaces.RealOutput yHea(
    final min=0,
    final max=1,
    final unit="1") "Heating control signal"
    annotation (Placement(transformation(extent={{160,-90},{200,-50}}),
      iconTransformation(extent={{100,-80},{140,-40}})));
  BuildingsLite.Controls.OBC.CDL.Reals.PIDWithReset conCoo(
    final controllerType=BuildingsLite.Controls.OBC.CDL.Types.SimpleController.PI,
    final k=kCooCon,
    final Ti=TiCooCon,
    final reverseActing=false)
    "Cooling controller"
    annotation (Placement(transformation(extent={{-80,90},{-60,110}})));
  BuildingsLite.Controls.OBC.CDL.Reals.PIDWithReset conHea(
    final controllerType=BuildingsLite.Controls.OBC.CDL.Types.SimpleController.PI,
    final k=kHeaCon,
    final Ti=TiHeaCon)
    "Heating controller"
...
protected
  BuildingsLite.Controls.OBC.CDL.Reals.Less enaHeaLoo(
    final h=dTHys)
    "Check if heating control loop should be enabled"
    annotation (Placement(transformation(extent={{-120,-130},{-100,-110}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Less enaCooLoo(
    final h=dTHys)
    "Check if cooling control loop should be enabled"
    annotation (Placement(transformation(extent={{-120,10},{-100,30}})));
  BuildingsLite.Controls.OBC.CDL.Logical.TrueDelay disCoo(
    final delayTime=timChe)
    "Check if the controller output has been near zero for a threshold time"
    annotation (Placement(transformation(extent={{0,50},{20,70}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Not colZon
    "Check if the zone temperature is lower than the cooling setpoint"
    annotation (Placement(transformation(extent={{-40,10},{-20,30}})));
  BuildingsLite.Controls.OBC.CDL.Conversions.BooleanToReal zerCoo(
    final realTrue=0,
    final realFalse=1)
    "Output zero control signal when the cooling loop should be disabled"
    annotation (Placement(transformation(extent={{80,50},{100,70}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Multiply cooConSig
    "Cooling control loop signal"
    annotation (Placement(transformation(extent={{120,60},{140,80}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Not holZon
    "Check if the zone temperature is higher than the heating setpoint"
    annotation (Placement(transformation(extent={{-40,-130},{-20,-110}})));
  BuildingsLite.Controls.OBC.CDL.Logical.TrueDelay disHea(
    final delayTime=timChe)
    "Check if the controller output has been near zero for a threshold time"
    annotation (Placement(transformation(extent={{0,-90},{20,-70}})));
  BuildingsLite.Controls.OBC.CDL.Conversions.BooleanToReal zerHea(
    final realTrue=0,
    final realFalse=1)
    "Output zero control signal when the heating loop should be disabled"
    annotation (Placement(transformation(extent={{80,-90},{100,-70}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Multiply heaConSig
    "Heating control loop signal"
    annotation (Placement(transformation(extent={{120,-80},{140,-60}})));
  BuildingsLite.Controls.OBC.CDL.Reals.LessThreshold zerCon(
    final t=looHys, final h=0.8*looHys)
    "Check if the controller output is near zero"
    annotation (Placement(transformation(extent={{-40,50},{-20,70}})));
  BuildingsLite.Controls.OBC.CDL.Reals.LessThreshold zerCon1(
    final t=looHys, final h=0.8*looHys)
    "Check if the controller output is near zero"
    annotation (Placement(transformation(extent={{-40,-90},{-20,-70}})));
  BuildingsLite.Controls.OBC.CDL.Logical.And disCooCon
    "Check if disable cooling control loop"
    annotation (Placement(transformation(extent={{40,50},{60,70}})));
  BuildingsLite.Controls.OBC.CDL.Logical.And disHeaCon
