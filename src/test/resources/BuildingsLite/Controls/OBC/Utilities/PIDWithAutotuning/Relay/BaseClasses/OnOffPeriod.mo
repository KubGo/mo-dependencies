...
protected
  BuildingsLite.Controls.OBC.CDL.Discrete.TriggeredSampler timOn
    "Simulation time when the input signal becomes on (True)"
    annotation (Placement(transformation(extent={{-50,30},{-30,50}})));
  BuildingsLite.Controls.OBC.CDL.Discrete.TriggeredSampler timOff
    "Simulation time when the input signal becomes off (False)"
    annotation (Placement(transformation(extent={{-50,-10},{-30,10}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Subtract lenOffCal
    "Block that calculates the horizon length for the off period"
    annotation (Placement(transformation(extent={{10,-50},{30,-30}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Subtract lenOnCal
  ...
  BuildingsLite.Controls.OBC.Utilities.PIDWithAutotuning.Relay.BaseClasses.SamplerWithResetThreshold timOnRec(
     final lowLim=0, final y_reset=0)
    "Sample the on time"
    annotation (Placement(transformation(extent={{60,30},{80,50}})));
  BuildingsLite.Controls.OBC.Utilities.PIDWithAutotuning.Relay.BaseClasses.SamplerWithResetThreshold timOffRec(
     final lowLim=0, final y_reset=0)
    "Sample the off time"
