...
block HalfPeriodRatio
  ...
  BuildingsLite.Controls.OBC.CDL.Interfaces.BooleanInput TunEnd
    "True: the tuning process ends"
    annotation (Placement(transformation(extent={{-140,-20},{-100,20}}),
        iconTransformation(extent={{-140,-20},{-100,20}})));
  BuildingsLite.Controls.OBC.CDL.Interfaces.RealOutput rho
    "Real signal of the half period ratio"
    annotation (Placement(transformation(extent={{100,-20},{140,20}}),
        iconTransformation(extent={{100,-20},{140,20}})));

protected
  BuildingsLite.Controls.OBC.CDL.Discrete.TriggeredSampler tOnSam(
    final y_start=BuildingsLite.Controls.OBC.CDL.Constants.eps)
    "Block that samples tOn when the tuning period ends"
    annotation (Placement(transformation(extent={{-80,70},{-60,90}})));
  BuildingsLite.Controls.OBC.CDL.Discrete.TriggeredSampler tOffSam(
    final y_start=BuildingsLite.Controls.OBC.CDL.Constants.eps)
    "Block that samples tOff when the tuning period ends"
    annotation (Placement(transformation(extent={{-60,-80},{-40,-60}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Min mintOntOff
    "Block that finds the smaller one between the length for the on period and the length for the off period"
    annotation (Placement(transformation(extent={{0,-50},{20,-30}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Max maxtOntOff
    "Block that finds the larger one between the length for the on period and the length for the off period"
    annotation (Placement(transformation(extent={{0,30},{20,50}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Divide halPerRat
