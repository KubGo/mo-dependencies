within BuildingsLite.Controls.OBC.RadiantSystems.Heating;
block HighMassSupplyTemperature_TRoom
  ...
  parameter Controls.OBC.CDL.Types.SimpleController controllerType=BuildingsLite.Controls.OBC.CDL.Types.SimpleController.P
    "Type of controller" annotation (Dialog(group="Control gains"));
  parameter Real k(
    min=100*BuildingsLite.Controls.OBC.CDL.Constants.eps)=2
    "Gain of controller"
    annotation (Dialog(group="Control gains"));
  parameter Real Ti(
    final quantity="Time",
    final unit="s",
    min=100*BuildingsLite.Controls.OBC.CDL.Constants.eps)=3600
    "Time constant of integrator block"
    annotation (Dialog(group="Control gains",enable=controllerType == BuildingsLite.Controls.OBC.CDL.Types.SimpleController.PI or controllerType == BuildingsLite.Controls.OBC.CDL.Types.SimpleController.PID));
  parameter Real Td(
    final quantity="Time",
    final unit="s",
    min=100*BuildingsLite.Controls.OBC.CDL.Constants.eps)=0.1
    "Time constant of derivative block"
    annotation (Dialog(group="Control gains",enable=controllerType == BuildingsLite.Controls.OBC.CDL.Types.SimpleController.PD or controllerType == BuildingsLite.Controls.OBC.CDL.Types.SimpleController.PID));
  ...
  BuildingsLite.Controls.OBC.CDL.Reals.PID conHea(
  ...
  BuildingsLite.Controls.OBC.CDL.Reals.Hysteresis hysHea(
    uLow=0.1,
    uHigh=0.2)
...
protected
  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Constant one(final k=1) "Outputs one"
    annotation (Placement(transformation(extent={{-40,-10},{-20,10}})));
  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Constant zero(final k=0) "Outputs zero"
    annotation (Placement(transformation(extent={{-40,60},{-20,80}})));

  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Constant THeaSup_minimum(final k(
      final unit="K",
      displayUnit="degC") = TSupSet_min)
    "Negative value of minimum heating supply water temperature"
    annotation (Placement(transformation(extent={{-40,30},{-20,50}})));

  BuildingsLite.Controls.OBC.CDL.Reals.Sources.Constant THeaSup_maximum(
    final k(
      final unit="K",
      displayUnit="degC") = TSupSet_max) "Maximum heating supply water temperature"
    annotation (Placement(transformation(extent={{-40,-40},{-20,-20}})));

  BuildingsLite.Controls.OBC.CDL.Reals.Line TSup(
    limitBelow=false,
    limitAbove=false,
    y(final unit="K",
