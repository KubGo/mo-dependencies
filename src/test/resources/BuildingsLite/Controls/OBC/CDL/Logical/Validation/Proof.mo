...
model Proof "Validation model for the Proof block"
  BuildingsLite.Controls.OBC.CDL.Logical.Proof pro(
    debounce=0.5,
    feedbackDelay=0.75) "Both inputs change at the same time"
    annotation (Placement(transformation(extent={{-20,70},{0,90}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Proof pro1(
    debounce=0.5,
    feedbackDelay=0.75)
    "Commanded input changes from true to false earlier than measured input"
    annotation (Placement(transformation(extent={{-20,30},{0,50}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Proof pro2(
    debounce=0.5,
    feedbackDelay=0.75)
    "Measured input changes from true to false earlier than commanded input"
    annotation (Placement(transformation(extent={{-20,-10},{0,10}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Proof pro3(
    debounce=0.5,
    feedbackDelay=0.5)
    "Shorter delay to valid input"
    annotation (Placement(transformation(extent={{-20,-50},{0,-30}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Proof pro4(
    debounce=0.5,
    feedbackDelay=0.5)
    "Shorter delay to valid input"
    annotation (Placement(transformation(extent={{-20,-90},{0,-70}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Proof pro5(
    debounce=2,
    feedbackDelay=2)
  ...
  BuildingsLite.Controls.OBC.CDL.Logical.Sources.Pulse latInp1(
    width=0.2,
    period=10,
    shift=1)
    "Block that outputs cyclic on and off"
    annotation (Placement(transformation(extent={{-80,10},{-60,30}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Sources.Pulse latInp2(
    width=0.1,
    period=10,
    shift=1)
    "Block that outputs cyclic on and off"
    annotation (Placement(transformation(extent={{-80,-60},{-60,-40}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Sources.Pulse latInp3(
    width=0.5,
    period=2,
    shift=1)
    "Block that outputs cyclic on and off"
    annotation (Placement(transformation(extent={{40,30},{60,50}})));
  BuildingsLite.Controls.OBC.CDL.Logical.Sources.Pulse latInp4(
    width=0.9,
    period=12,
    shift=0.5)
    "Block that outputs cyclic on and off"
...
<a href=\"modelica://BuildingsLite.Controls.OBC.CDL.Logical.Proof\">
BuildingsLite.Controls.OBC.CDL.Logical.Proof</a>. The following tests are implemented:
</p>
<ul>
<li>
