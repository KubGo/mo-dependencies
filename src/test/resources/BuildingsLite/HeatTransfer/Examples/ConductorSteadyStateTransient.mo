within BuildingsLite.HeatTransfer.Examples;
model ConductorSteadyStateTransient "Test model for heat conductor"
  extends Modelica.Icons.Example;

  parameter BuildingsLite.HeatTransfer.Data.Solids.Brick brick(x=0.12, nStaRef=4)
    annotation (Placement(transformation(extent={{40,60},{60,80}})));
  parameter BuildingsLite.HeatTransfer.Data.Solids.InsulationBoard insul(
    x=0.05,
    c=0,
    nStaRef=3) "Insulation"
    annotation (Placement(transformation(extent={{0,60},{20,80}})));
  parameter BuildingsLite.HeatTransfer.Data.OpaqueConstructions.Generic composite(nLay=2, material=
       {insul,brick})
    annotation (Placement(transformation(extent={{80,60},{100,80}})));

  BuildingsLite.HeatTransfer.Conduction.MultiLayer conMul(
    A=2, layers=composite,
    stateAtSurface_b=false)
    annotation (Placement(transformation(extent={{40,-60},{60,-40}})));
  BuildingsLite.HeatTransfer.Conduction.SingleLayer con(
    A=2, material=brick,
    stateAtSurface_a=false,
    stateAtSurface_b=false)
                         annotation (Placement(transformation(extent={{20,20},{40,40}})));

  BuildingsLite.HeatTransfer.Sources.FixedTemperature TB(T=293.15)
    annotation (Placement(transformation(extent={{80,20},{60,40}})));
  BuildingsLite.HeatTransfer.Sources.PrescribedTemperature TA
  ...
  BuildingsLite.HeatTransfer.Conduction.SingleLayer con1(
    A=2, material=insul,
    stateAtSurface_b=false)
         annotation (Placement(transformation(extent={{20,-20},{40,0}})));
  BuildingsLite.HeatTransfer.Sources.FixedTemperature TB1(T=293.15)
    annotation (Placement(transformation(extent={{100,-20},{80,0}})));
  BuildingsLite.HeatTransfer.Sources.PrescribedTemperature TA1
    annotation (Placement(transformation(extent={{-60,-20},{-40,0}})));
  BuildingsLite.HeatTransfer.Conduction.SingleLayer con2(
    A=2, material=brick,
    stateAtSurface_b=false,
    stateAtSurface_a=false)
             annotation (Placement(transformation(extent={{50,-20},{70,0}})));
  BuildingsLite.HeatTransfer.Sources.PrescribedTemperature TA2
    annotation (Placement(transformation(extent={{-60,-60},{-40,-40}})));
  BuildingsLite.HeatTransfer.Sources.FixedTemperature TB2(T=293.15)
  ...
  BuildingsLite.HeatTransfer.Convection.Interior conv1(A=2, til=BuildingsLite.Types.Tilt.Wall)
    "Convective heat transfer"
    annotation (Placement(transformation(extent={{-10,-20},{-30,0}})));
  BuildingsLite.HeatTransfer.Convection.Interior conv2(A=2, til=BuildingsLite.Types.Tilt.Wall)
    "Convective heat transfer"
