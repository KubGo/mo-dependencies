...
model ConductorInitialization "Test model for heat conductor initialization"
  ...
  BuildingsLite.HeatTransfer.Sources.FixedTemperature TB(T=303.15)
    annotation (Placement(transformation(extent={{80,20},{60,40}})));
  BuildingsLite.HeatTransfer.Sources.PrescribedTemperature TA
    annotation (Placement(transformation(extent={{-60,20},{-40,40}})));

  parameter BuildingsLite.HeatTransfer.Data.Solids.Brick brick(x=0.18, nStaRef=3,
    nSta=5)
    annotation (Placement(transformation(extent={{18,72},{38,92}})));
  parameter BuildingsLite.HeatTransfer.Data.Solids.InsulationBoard insulation(x=0.05, nStaRef=2,
    nSta=5)
    annotation (Placement(transformation(extent={{-20,72},{0,92}})));
  parameter BuildingsLite.HeatTransfer.Data.OpaqueConstructions.Generic compositeWall(
  ...
  BuildingsLite.HeatTransfer.Conduction.MultiLayer conS1(
    A=2,
