within BuildingsLite.HeatTransfer.Windows.Examples;
model Window "Test model for the window"
  ...
  BuildingsLite.HeatTransfer.Windows.Window window(
    A=A,
    fFra=fFra,
    glaSys=glaSys,
    linearize=linearize,
  ...
  BuildingsLite.HeatTransfer.Windows.ExteriorHeatTransfer extCon(
    A=A,
    fFra=fFra,
  ...
  BuildingsLite.HeatTransfer.Sources.PrescribedTemperature TOuts
    "Outside air temperature"
    annotation (Placement(transformation(extent={{-20,40},{0,60}})));
  BuildingsLite.HeatTransfer.Sources.FixedTemperature TRAir(T=293.15)
    "Room air temperature"
    annotation (Placement(transformation(extent={{300,20},{280,40}})));
  BuildingsLite.HeatTransfer.Radiosity.IndoorRadiosity indRad(A=A)
    "Model for indoor radiosity"
    annotation (Placement(transformation(extent={{322,100},{302,120}})));
  BuildingsLite.HeatTransfer.Sources.FixedHeatFlow fixedHeatFlow(Q_flow=0)
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=90,
        origin={312,68})));
  replaceable parameter
    BuildingsLite.HeatTransfer.Data.GlazingSystems.DoubleClearAir13Clear glaSys(
    shade=BuildingsLite.HeatTransfer.Data.Shades.Gray(),
    haveExteriorShade=false,
    haveInteriorShade=true) constrainedby Data.GlazingSystems.Generic
    annotation (Placement(transformation(extent={{-40,140},{-20,160}})));
  BuildingsLite.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirTil(
    til=til,
    azi=azi)
    annotation (Placement(transformation(extent={{60,-20},{80,0}})));
  BuildingsLite.BoundaryConditions.SolarIrradiation.DiffuseIsotropic HDifTilIso(
    til=til)
    annotation (Placement(transformation(extent={{60,20},{80,40}})));
  BuildingsLite.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    filNam=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/weatherdata/USA_IL_Chicago-OHare.Intl.AP.725300_TMY3.mos"))
    annotation (Placement(transformation(extent={{-20,-20},{0,0}})));
  Modelica.Blocks.Math.Gain HRoo(k=0.1) "Solar irradiation from room"
    annotation (Placement(transformation(extent={{60,-60},{80,-40}})));
  BuildingsLite.HeatTransfer.Windows.BaseClasses.WindowRadiation winRad(
    AWin=1,
  ...
  BuildingsLite.HeatTransfer.Windows.BaseClasses.ShadeRadiation intShaRad(
  ...
  BuildingsLite.HeatTransfer.Windows.BaseClasses.ShadingSignal shaSig(
  ...
  BuildingsLite.HeatTransfer.Windows.InteriorHeatTransferConvective intShaCon(
    A=A,
    fFra=fFra,
    til=BuildingsLite.Types.Tilt.Wall,
    haveExteriorShade=glaSys.haveExteriorShade,
    haveInteriorShade=glaSys.haveInteriorShade)
...
protected
  ...
  BuildingsLite.HeatTransfer.Radiosity.RadiositySplitter radShaOut
