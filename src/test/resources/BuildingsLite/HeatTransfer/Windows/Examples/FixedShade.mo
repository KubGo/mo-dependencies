...
model FixedShade "Test model for the fixed shade model"
  ...
  BuildingsLite.HeatTransfer.Windows.FixedShade sha[4](
    final conPar=conPar,
  ...
  BuildingsLite.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirTil(
    ...
    azi=BuildingsLite.Types.Azimuth.S) "Direct solar irradiation"
  ...
  parameter BuildingsLite.ThermalZones.Detailed.BaseClasses.ParameterConstructionWithWindow conPar[4](
    each til=BuildingsLite.Types.Tilt.Wall,
    each azi=BuildingsLite.Types.Azimuth.S,
    each A=20,
    each hWin=1.5,
    each wWin=2,
    each glaSys=glaSys,
