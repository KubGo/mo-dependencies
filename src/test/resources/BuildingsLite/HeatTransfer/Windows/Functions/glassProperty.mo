  ...
  for i in 1:N loop
    ...
    for iSta in 1:NSta loop
      ...
      if (abs(glass[Ra, i, iSta] - glass[Rb, i, iSta]) < tol) then
        oneLay[:,:,iSta] := BuildingsLite.HeatTransfer.Windows.Functions.glassPropertyUncoated(
          HEM,
          oneGla[:,iSta],
          xGla[i],
          psi);
        ...
        //coated glass
        oneLay[:,:,iSta] := BuildingsLite.HeatTransfer.Windows.Functions.glassPropertyCoated(
          HEM,
          oneGla[:, iSta],
          psi);
       end if;
