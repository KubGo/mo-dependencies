...
block SideFins
  ...
  extends BuildingsLite.ThermalZones.Detailed.BaseClasses.SideFins;
...
equation
  ...
  //avoiding division by zero
    lambda_t = BuildingsLite.Utilities.Math.Functions.smoothMax(
      x1=tanLambda,
      x2=delta,
      deltaX=delta/10);
    verAzi_t = BuildingsLite.Utilities.Math.Functions.smoothMax(
      x1=Modelica.Math.tan(verAzi),
      x2=delta,
      deltaX=delta/10);
    verAzi_c = BuildingsLite.Utilities.Math.Functions.smoothMax(
      x1=Modelica.Math.cos(verAzi),
      x2=delta,
      deltaX=delta/10);
    alt_t = BuildingsLite.Utilities.Math.Functions.smoothMax(
    ...
    for i in 1:4 loop
      ...
      minX2X3[i] = BuildingsLite.Utilities.Math.Functions.smoothMin(
        x1=x2,
        x2=x3[i],
        deltaX=deltaL);
      minX[i] = BuildingsLite.Utilities.Math.Functions.smoothMin(
        x1=x1[i],
        x2=minX2X3[i],
        deltaX=deltaL);
      minY2Y3[i] = BuildingsLite.Utilities.Math.Functions.smoothMin(
        x1=y2,
        x2=y3[i],
        deltaX=deltaL);
      minY[i] = BuildingsLite.Utilities.Math.Functions.smoothMin(
        x1=y1[i],
        x2=minY2Y3[i],
        deltaX=deltaL);
  ...
  //correction case: Sun below horizon
    crShdArea2 = BuildingsLite.Utilities.Math.Functions.spliceFunction(
       ...
       x=alt,
       deltax=0.01);
    crShdArea=BuildingsLite.Utilities.Math.Functions.smoothMax(
       x1=crShdArea1,
       x2=crShdArea2,
       deltaX=0.0001*AWin);
