...
block Overhang
  ...
  extends BuildingsLite.ThermalZones.Detailed.BaseClasses.Overhang;
...
// Window dimensions
  ...
  BuildingsLite.BoundaryConditions.WeatherData.Bus weaBus "Weather data"
...
protected
  ...
  BuildingsLite.BoundaryConditions.SolarGeometry.BaseClasses.SolarAzimuth solAzi
    "Solar azimuth"
...
equation
  ...
  // BuildingsLite.HeatTransfer.Windows.FixedShade in case the window has no overhang.
  ...
  if haveOverhang then
    ...
    w = BuildingsLite.Utilities.Math.Functions.spliceFunction(
            pos=wL,
            neg=wR,
            x=solAzi.solAzi-azi,
            deltax=delSolAzi);
    ...
    for i in 1:4 loop
      ...
      x2[i] = x1*tmpH[i]/BuildingsLite.Utilities.Math.Functions.smoothMax(
        x1=y1, x2=1E-8*hWin, deltaX=1E-9*hWin);
      area[i] = BuildingsLite.Utilities.Math.Functions.smoothMin(
        x1=y1,
        x2=y2[i],
          deltaX=del_L)*tmpW[i] - (BuildingsLite.Utilities.Math.Functions.smoothMin(
        y1,
        tmpH[i],
        del_L)*BuildingsLite.Utilities.Math.Functions.smoothMin(
        x1=x2[i],
        x2=y1,
        deltaX=del_L)/2) + BuildingsLite.Utilities.Math.Functions.smoothMax(
        x1=shdwTrnglRtio*(BuildingsLite.Utilities.Math.Functions.smoothMin(
          x1=x1,
          x2=x2[i],
          deltaX=del_L) - tmpW[i]),
        x2=0,
        deltaX=del_L)*BuildingsLite.Utilities.Math.Functions.smoothMax(
        x1=(BuildingsLite.Utilities.Math.Functions.smoothMin(
          x1=x1,
          x2=x2[i],
          deltaX=del_L) - tmpW[i]),
        x2=0,
  ...
  // correction case: Sun not in front of the wall
    crShdArea1 = BuildingsLite.Utilities.Math.Functions.spliceFunction(
      pos=shdArea,
      neg=AWin,
      x=(Modelica.Constants.pi/2)-verAzi,
      deltax=0.01);
  // correction case: Sun not above horizon
    crShdArea2 = BuildingsLite.Utilities.Math.Functions.spliceFunction(
      pos=shdArea,
      neg=AWin,
      x=alt,
      deltax=0.01);
    crShdArea=BuildingsLite.Utilities.Math.Functions.smoothMax(x1=crShdArea1,
                                                           x2=crShdArea2,
                                                           deltaX=0.01);
    fraSun = BuildingsLite.Utilities.Math.Functions.smoothMin(
        x1=BuildingsLite.Utilities.Math.Functions.smoothMax(x1=1-crShdArea/AWin,x2=0,deltaX=0.01),
        x2=1.0,
        deltaX=0.01);
  else
    w = 0;
...
<a href=\"modelica://BuildingsLite.BoundaryConditions.SolarGeometry.BaseClasses\">
BuildingsLite.BoundaryConditions.SolarGeometry.BaseClasses</a>.
</p>
<p>
