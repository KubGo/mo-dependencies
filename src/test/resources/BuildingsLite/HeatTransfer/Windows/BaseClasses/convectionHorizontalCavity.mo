...
function convectionHorizontalCavity "Free convection in horizontal cavity"
  ...
  input BuildingsLite.HeatTransfer.Data.Gases.Generic gas
...
algorithm
  if cosTil > 0 then
  Nu :=BuildingsLite.Utilities.Math.Functions.spliceFunction(
    pos=
      BuildingsLite.HeatTransfer.Windows.BaseClasses.nusseltHorizontalCavityReduced(
      gas=gas,
      Ra=Ra,
      T_m=T_m,
      dT=dT,
      ...
      deltaRa=deltaRa),
    neg=
      BuildingsLite.HeatTransfer.Windows.BaseClasses.nusseltHorizontalCavityEnhanced(
      gas=gas,
      Ra=Ra,
      T_m=T_m,
      dT=dT,
  ...
  hCon :=Nu*BuildingsLite.HeatTransfer.Data.Gases.thermalConductivity(gas, T_m)/gas.x;
  q_flow :=hCon*dT;
