...
algorithm
  ...
  Nu_1 :=BuildingsLite.Utilities.Math.Functions.spliceFunction(
    pos=0.0673838*Ra^(1/3),
    neg=BuildingsLite.Utilities.Math.Functions.spliceFunction(
  ...
  */
  ...
  Nu :=BuildingsLite.Utilities.Math.Functions.smoothMax(
    x1=Nu_1,
    x2=Nu_2,
    deltaX=deltaNu);
  hCon :=Nu*BuildingsLite.HeatTransfer.Data.Gases.thermalConductivity(gas=gas, T=T_m)/gas.x;
  q_flow :=hCon*dT;
