within BuildingsLite.HeatTransfer.Convection;
model Exterior "Model for a exterior (outside) convective heat transfer"
  ...
  parameter BuildingsLite.HeatTransfer.Types.ExteriorConvection conMod=
    BuildingsLite.HeatTransfer.Types.ExteriorConvection.TemperatureWind
  ...
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer hFixed=3
    "Constant convection coefficient" annotation (Dialog(enable=(conMod ==
          BuildingsLite.HeatTransfer.Types.ExteriorConvection.Fixed)));

  parameter BuildingsLite.HeatTransfer.Types.SurfaceRoughness roughness=
    BuildingsLite.HeatTransfer.Types.SurfaceRoughness.Medium "Surface roughness"
    annotation (Dialog(enable=(conMod <> BuildingsLite.HeatTransfer.Types.ExteriorConvection.Fixed)));
...
initial equation
  if (roughness == BuildingsLite.HeatTransfer.Types.SurfaceRoughness.VeryRough) then
    R=2.17;
  elseif (roughness == BuildingsLite.HeatTransfer.Types.SurfaceRoughness.Rough) then
    R=1.67;
  elseif (roughness == BuildingsLite.HeatTransfer.Types.SurfaceRoughness.Medium) then
    R=1.52;
  elseif (roughness == BuildingsLite.HeatTransfer.Types.SurfaceRoughness.MediumSmooth) then
    R=1.13;
  elseif (roughness == BuildingsLite.HeatTransfer.Types.SurfaceRoughness.Smooth) then
    R=1.11;
  elseif (roughness == BuildingsLite.HeatTransfer.Types.SurfaceRoughness.VerySmooth) then
    R=1.00;
  else
    R=0;
  end if;
equation
  if (conMod == BuildingsLite.HeatTransfer.Types.ExteriorConvection.Fixed) then
    qN_flow = hFixed * dT;
    W = 1;
    hF = 0;
    qF_flow = 0;
    ...
    if is_ceiling then
       qN_flow = BuildingsLite.HeatTransfer.Convection.Functions.HeatFlux.ceiling(dT=dT);
    elseif is_floor then
       qN_flow = BuildingsLite.HeatTransfer.Convection.Functions.HeatFlux.floor(dT=dT);
    else
       qN_flow = BuildingsLite.HeatTransfer.Convection.Functions.HeatFlux.wall(dT=dT);
    end if;
    // Forced convection
    W = BuildingsLite.Utilities.Math.Functions.regStep(
          x = v-v_small/2,
          y1 = BuildingsLite.HeatTransfer.Convection.Functions.windDirectionModifier(
            azi=azi,
            dir=dir),
          y2 = 0.75,
          x_small=v_small/4);
    hF = 2.537 * W * R * 2 / A^(0.25) *
       BuildingsLite.Utilities.Math.Functions.regNonZeroPower(
           x=v,
           n=0.5,
           delta=v_small);
    qF_flow = hF*dT;
...
<ol>
<li><p>If <code>conMod=
<a href=\"modelica://BuildingsLite.HeatTransfer.Types.ExteriorConvection\">
BuildingsLite.HeatTransfer.Types.ExteriorConvection.Fixed</a>
</code>, then
...
If <code>conMod=
<a href=\"modelica://BuildingsLite.HeatTransfer.Types.ExteriorConvection\">
BuildingsLite.HeatTransfer.Types.ExteriorConvection.TemperatureWind</a>
</code>,
...
<a href=\"modelica://BuildingsLite.HeatTransfer.Types.SurfaceRoughness\">
BuildingsLite.HeatTransfer.Types.SurfaceRoughness</a>.The coefficients for the surface roughness are
</p>
